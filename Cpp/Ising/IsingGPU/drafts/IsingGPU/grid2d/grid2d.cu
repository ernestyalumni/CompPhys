...
/* =============== device GPU structs =============== */ 
/* change in energy Delta E as local energy at each site */ 

// default constructor
DeltaE2d::DeltaE2d() {}

// constructor  
DeltaE2d::DeltaE2d(std::array<size_t,2> & L_is) : L_is {L_is}  { 

	std::unique_ptr<float[], deleterRR_struct> d_E(nullptr, deleterRR_struct() ); 
	cudaMallocManaged((void **) &d_E, L_is[0]*L_is[1] * sizeof(float)) ;
	DeltaE = std::move(d_E);  
	cudaMemset(DeltaE.get(), 0.f, L_is[0]*L_is[1] * sizeof(float));
} 


// move constructor 
DeltaE2d::DeltaE2d( DeltaE2d && old_DeltaE2d) : 
	L_is {old_DeltaE2d.L_is},  
	DeltaE { std::move( old_DeltaE2d.DeltaE ) } { }  
	
// operator overload assignment = 
DeltaE2d & DeltaE2d::operator=(DeltaE2d && old_DeltaE2d) {
	L_is = old_DeltaE2d.L_is;
	DeltaE = std::move( old_DeltaE2d.DeltaE ); 
	return *this;  
}
