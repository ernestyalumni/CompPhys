...
__global__ void metropolis_kernel(int* Sptr, Sysparam* sysparams,Avg* avgs,float* transprob, size_t Lx, size_t Ly, const float J,
									curandState *state) 
{
...
void metropolis(Spins2d& spins2d, Sysparam_ptr& sysParams,Avg_ptr& averages,TransProb_ptr& transProbs,
	...
	for (int cycles=1; cycles <= trials; cycles++) { 
		...
		metropolis_kernel<<< N_is,M_is,sharedBytes>>>( spins2d.S.get(), sysParams.d_sysparams.get(), 
			averages.d_avgs.get(), (transProbs.d_transProb.get()->transProb).data(), Lx,Ly, J, 
...
//	int sharedBytes = M_x * sizeof(int);  
	...
	metropolis(spins,sysparams_ptr,avgs_ptr,transprob_ptr,MAXGRIDSIZES,M_is,devstatesXOR,10000); 
