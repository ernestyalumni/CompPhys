...
//	GPUAnimBitmap  *bitmap = d->bitmap;
	...
	/*
	 * chosen 90 time steps per frame; manually ("experimentally") determined as 
	 * reasonable tradeoff between having to download a bitmap image for every time step and 
	 * computing too many time steps per frame, resulting in a jerky animation
	 * you could change this, if more concerned with getting output of each simulation step
	 * than animating results in real time, e.g. only single step on each frame
	 * cf. pp. 123 of Sanders and Kandrot
	 */	
	for (int i = 0; i < 90; i++) {
