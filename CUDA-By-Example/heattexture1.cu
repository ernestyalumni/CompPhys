 ...
 * cf. pp. 127 of Sanders and Kandrot */
...
__global__ void blend_kernel( float *dst, bool dstOut ){
	...
	/* tex1Dfetch looks like a function, but it's a compilier intrinsic 
	 * AND compiler needs to know at compile time which textures tex1Dfetch
...
void anim_exit( DataBlock *d) {
	// we not only have to free the global buffers, but we also need to unbind textures
