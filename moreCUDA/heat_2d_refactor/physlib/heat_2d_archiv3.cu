...
__global__ void tempKernel(float *d_temp, BC bc) {
	...
	// Load halo cells
	if (threadIdx.x < RAD ) {
		s_in[flatten(s_x - RAD, s_y, S_x, S_y)] = 
			d_temp[flatten(k_x - RAD, k_y, dev_Ld[0], dev_Ld[1])];
		s_in[flatten(s_x + blockDim.x, s_y, S_x, S_y)] = 
			d_temp[flatten(k_x + blockDim.x, k_y, dev_Ld[0], dev_Ld[1])];
	}
	if (threadIdx.y < RAD) {
		s_in[flatten(s_x, s_y - RAD, S_x, S_y)] = 
			d_temp[flatten(k_x, k_y - RAD, dev_Ld[0], dev_Ld[1])];
		s_in[flatten(s_x, s_y + blockDim.y, S_x, S_y)] = 
...
//	__shared__ float2 stencil[NUS][2] ; // doesn't work
