...
__global__ void float_to_char( uchar4* dev_out, const float* outSrc) {
	...
	const int zcoordslice = dev_Ld[2]/2*1; 
...
void kernelLauncher(uchar4 *d_out, float *d_temp, dim3 Ld, BC bc, dim3 M_in) {
	...
	float_to_char<<<out_gridSize,out_M>>>(d_out, d_temp) ; 
}

void kernelLauncher2(uchar4 *d_out, float *d_temp, dim3 Ld, BC bc, dim3 M_in) {
	constexpr int radius { 2 };
	
	const dim3 gridSize(blocksNeeded(Ld.x, M_in.x), blocksNeeded(Ld.y, M_in.y),
						blocksNeeded(Ld.z,M_in.z));
	const size_t smSz = (M_in.x + 2 * radius)*(M_in.y + 2 * radius)*(M_in.z + 2*radius)*sizeof(float);
...
void kernelLauncher3(uchar4 *d_out, float *d_temp, dim3 Ld, BC bc, dim3 M_in) {
...
void kernelLauncher4(uchar4 *d_out, float *d_temp, dim3 Ld, BC bc, dim3 M_in) {
