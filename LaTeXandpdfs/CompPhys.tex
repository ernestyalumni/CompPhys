...
Thus, given $S, \Omega_{S\to S}$ (all possible state transitions, not all permutations), random variable $X = X: \Omega_{S \to S} \to E$, where $E = \mathbb{R}$ (usually), and $P$ s.t. 
\begin{equation}
\text{Pr}(X \in S) = P(\lbrace \omega \in \Omega | X(\omega) \in S\rbrace) \text{ with } S \subseteq E
\end{equation}
then
\begin{equation}
